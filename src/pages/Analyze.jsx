...
import { buildStockPrompt } from '../lib/gptPromptBuilder';
...
const Analyze = () => {
  ...
  // 컴포넌트 마운트 시 티커에 해당하는 종목 정보 로드
  useEffect(() => {
    if (!ticker) {
      setError('종목 티커가 제공되지 않았습니다.');
      return;
    }
  ...
  useEffect(() => {
    const checkSavedReport = async () => {
      ...
      try {
        const report = await getReport(currentUser.uid, 'stock', ticker);
        if (report) {
  ...
  const handleRequestAnalysis = async () => {
    ...
    try {
      // 프롬프트 생성
      const prompt = buildStockPrompt(stock);
      
      // GPT API 호출
      const responseText = await callGptApi(prompt);
      
      // 응답 파싱
      const parsedResponse = parseGptResponse(responseText, ticker, 'stock');
      
      // 결과 설정
      setGptReport(parsedResponse);
      
      // Firestore에 저장
      await saveReport(currentUser.uid, 'stock', ticker, parsedResponse);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8 text-center">종목 GPT 분석</h1>
